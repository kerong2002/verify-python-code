    ...
    with open('test_output.txt','r') as test:
        for answer,test in zip(answer.readlines(),test.readlines()):
            ...
            if (answer!=test):#verify the test output
                print(test,', This output is not correct. Correct output is: ',answer,file = open("verify_result.txt", "a"))
                x=1
            else:
                print('pass',file = open("verify_result.txt", "a"))
        if(x==1):
            print('The output have some problems.',file = open("verify_result.txt", "a"))
        else:
